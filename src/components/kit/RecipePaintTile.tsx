...
export function RecipePaintTile({
  ...
  const content = (
    ...
    >
      <span className="truncate font-osd text-[8px] uppercase tracking-[0.08em] opacity-90">
        {brand ?? " "}
      </span>
      <span className="line-clamp-2 break-words font-mono text-[10px] font-semibold">
        {name}
      </span>
      <span className="truncate font-osd text-[8px] uppercase tracking-[0.08em] opacity-90">
        {layer || " "}
