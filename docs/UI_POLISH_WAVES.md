...
- [ ] **DOP-007a** — Rename nav label "RECIPE" → "RECIPES" (src/components/shell/NavLinks.tsx) for consistency with the page.
- [ ] **DOP-007b** — Add one-line `//`-comment-style descriptor under each page title where comprehension breaks (Library = "every paint on the market — mark what you own/want", Collection = "only the pots & models you own", plus Focus/Tools/Recipes one-liners). Reuse existing page-header component; keep on-aesthetic, single line. Acceptance: each listed page renders a one-line descriptor; no layout regression.
