...
- [ ] **UX-R7-001 — Custom wheel "Use this colour" silently drops the selection.**
...
- [ ] **UX-R7-002 — "EDIT SLOT" panel ambiguates "replace paint" vs "add step."**
...
- [ ] **UX-R7-004** — `/tools/wheel?name=...` deep-link doesn't pre-seed the wheel. Switch to `?hex=` query param + read it in the WheelClient mount.
- [ ] **UX-R7-005** — Form input borders at 1.4:1 fail WCAG 1.4.11. Lift `--color-border-input` (or wherever input borders resolve) to `#404040`.
- [ ] **UX-R7-006** — Cyan resurfaced on 10+ buttons (ATTACH RECIPE / ASSIGN TO PROJECT / ADD NAMED MODEL / CHANGE PASSWORD / SAVE FILTER / USE CAMERA / FIND IN LIBRARY / SEND TO RECIPE). Phase-12 cyan-on-buttons ban regression. Sweep these to the right semantic (green for ADD/SAVE/ATTACH, yellow for SHARE, etc).
- [ ] **UX-R7-007** — Six "Untitled recipe" rows after three sessions. No name prompt on NEW RECIPE. Add a required-name prompt on creation OR auto-prompt to rename inline.
- [ ] **UX-R7-008** — Project title not inline-editable. The recipe editor's `<h1>` is contenteditable; project detail's `<h1>` (same visual treatment) is plain text. Spread the pattern.
- [ ] **UX-R7-009** — Mobile users have no path to `/user`. Top-right avatar pill in MobileHeader doesn't navigate. Wrap it in a `<Link href="/user">`.
- [ ] **UX-R7-010** — Wishlist "Tools ▾" collides with sidebar Tools page name. Rename to "USE WITH ▾" or "OPEN IN ▾".
- [ ] **UX-R7-011** — Disclosure caret 20×20px fails WCAG 2.5.8 24px floor. Bump to 36×36; add `aria-expanded` + rotate-on-expand.
- [ ] **UX-R7-013** — Decorative "02" at 1.06:1 contrast, 96px font, no semantic meaning (library top-right). Drop or shrink to chrome scale.
