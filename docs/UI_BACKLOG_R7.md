...
- [x] **R7-1 — Project table inline editing.** → 4f33305 `/projects` table currently lets the user navigate to a project to edit its info. Ross wants direct table-cell editing:
...
- [x] **R7-2 — App-wide small-button sweep.** → 7ec3176 Cross-cutting size discipline. Every `<Button size="md">` and `<Button size="lg">` audited and demoted to `sm` UNLESS it's:
...
- [x] **R7-3 — ColorPicker brightness/lightness slider.** → 17429cc The mini color wheel in the ColorPicker primitive currently lets the user pick hue + saturation but not lightness. Ross wants the ability to pick darker colors. Add a vertical or horizontal lightness slider (range 0-100) below the wheel + harmony dropdown. Output hex updates as the slider moves. The harmony swatches also re-render at the new lightness.

- [x] **R7-4 — Tools color assignment UX (Match + Layering).** → 18a9fa1 Both tools currently render a fixed color palette but the user can't TELL them what color to start from. Fix:
...
- [x] **R7-5 — Library top-right "random filter button" verification.** → e704300 Ross reports a "random filter button that does nothing" in the top-right of `/library`. Locate the actual element:
...
- [x] **R7-6 — Surface check on app-wide button discipline.** → 95c7bda After R7-2 sweep, audit each top-level page for cyan buttons that survived the Phase 12 P12.24 pass. Cyan is banned from buttons. Common offenders to grep: `bg-[var(--color-cyan)]`, `btn-primary` consumers, ad-hoc `text-cyan` buttons. Flip each to the right semantic (green/yellow/red/purple).

- [x] **R7-7 — Recipes page button sweep.** → 3380418 Ross approved the recipe redesign but flagged "buttons should be smaller and look more like the UI example image buttons." Audit `/recipes` + `/recipes/[id]` + the new recipe-action bar. Apply R7-2 sweep treatment to every button on the recipe surfaces. The `Save` + `Assign to project` buttons stay prominent (size sm but visually loud via their tone).
